...
#%% Sequential Optimizayion

class SequentialOptimizationADAMSaver(ABC):
    def __init__(self,
                 M:List[LorentzFinslerManifold],
                 init_fun:Callable[[Array, Array, int], Array]=None,
                 lr_rate:float=0.01,
    ...
    def for_step(self,
        ...
        z_tacks, grad, opt_state = carry
        
        opt_state = self.opt_update(idx, grad, opt_state)
        z_tacks = self.get_params(opt_state)
        grad = self.Dtime(z_tacks)
        
        return ((z_tacks, grad, opt_state),)*2
    ...
    def __call__(self, 
        ...
        _, (z_tacks, *_) = lax.scan(self.for_step,
                                    init = (z_tacks, grad, opt_state),
                                    xs = jnp.ones(self.max_iter),
                                    )

        t, zt = vmap(self.tack_curve)(z_tacks)

        return t, zt, grad, None
