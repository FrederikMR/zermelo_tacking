...
class PointcarreNorthLeft(LorentzFinslerManifold):
    ...
    def F_metric(self, t, z, dz):
        ...
        return 2.*(0.5*jnp.sqrt((3*(x**2))+2*x*y+3*(y**2))-0.5*x-0.5*y)/(jnp.arctan(5*v-15)+jnp.arctan(20)+0.23)#*jnp.arctan(v))
...
class PointcarreNorthRight(LorentzFinslerManifold):
    ...
    def F_metric(self, t, z, dz):
        ...
        return 2.*(0.5*jnp.sqrt((3*(x**2))-2.*x*y+3*(y**2))-0.5*x+0.5*y)/(jnp.arctan(5*v-15)+jnp.arctan(20)+0.2)#3*jnp.arctan(v))
