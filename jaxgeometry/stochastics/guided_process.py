...
def get_guided(M:object,
    ...
    def _log_p_T(guided:Callable,
        ...
        # sample
        log_varphis = vmap(lambda dW: guided(x,v,dts,dW,*ys)[4][-1],1)(dW)
        
        log_varphi = jnp.log(jnp.mean(jnp.exp(log_varphis)))
        #(_,_,X,*_) = sde((T,v,chart,*cy),y)
        _logdetA = logdetA(x,*ys) if logdetA is not None else -2*jnp.linalg.slogdet(X)[1] #ERROR IF logdetA is not None
        log_p_T = .5*_logdetA-.5*x[0].shape[0]*jnp.log(2.*jnp.pi*T)-Cxv/(2.*T)+log_varphi
        
        return log_p_T
    
    guided = jit(lambda x,v,dts,dWs,*ys: integrate_sde(sde_guided,integrator_ito if integration == 'ito' else integrator_stratonovich,
                                                       chart_update_guided,x[0],x[1],dts,dWs,0.,0.,jnp.sum(dts),
                                                       M.update_coords(v,x[1])[0] if chart_update else v,*ys)[0:5])
    
    
    log_p_T = partial(_log_p_T,guided,A,phi)

    neg_log_p_Ts = lambda *args: -jnp.mean(vmap(lambda x,chart,w,dW,dts,*ys: log_p_T((x,chart),w,dW,dts,*ys),
                                                (None,None,0,0,None,*((None,)*(len(args)-5))))(*args))
